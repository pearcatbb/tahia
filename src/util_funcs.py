...
def print_train_log(epoch, dur, loss, train_f1, val_f1):
    # print(f"Epoch {epoch:05d} | Time(s) {np.mean(dur):.4f} | Loss {loss.item():.4f} | TrainF1 {train_f1:.4f} | ValF1 {val_f1:.4f}")
    print(f"Epoch {epoch:05d} | Time(s) {np.mean(dur):.4f} | Loss {loss.item():.4f}")
...
def restart_random_walk(adj, steps, types, lens, target_type, e):
    # adj = torch.zeros_like(adj_matrix)
    # adj[torch.where(adj_matrix > 0)] = 1
    neighbors = [[] for _ in range(len(types))]
    for i in types[target_type]['ind']:
        neighbor = [[] for _ in range(len(types))]
        neighbor[0].append(i)
        cur_node = i
        for j in range(steps):
            ...
            if rand_p < 0.1:
                cur_node = i
                neighbor[0].append(cur_node)
                continue
            cur_nei = torch.where(adj[cur_node] > 0)[0]
            cur_node = int(random.choice(cur_nei))
            for t, (k, v) in enumerate(types.items()):
                if cur_node < max(list(v['ind'])):
                    neighbor[t].append(cur_node)
                    break
        for t in range(len(types)):
            if len(neighbor[t]) == 0:
                neighbor[t] = [-1]
            neighbor[t] = neighbor[t] * lens[t]
            neighbor[t] = neighbor[t][: lens[t]]
            neighbors[t].append(neighbor[t])
    current_path = os.getcwd()
    with open(current_path + '/results/credit/HGSL/random_walk/epoch' + str(e) + '.pkl', 'wb') as file:
